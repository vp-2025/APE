...
UINT CodePageFromCharSet(DWORD characterSet, UINT documentCodePage) {
	...
	switch (characterSet) {
	case SC_CHARSET_ANSI: return 1251; // vp: 1252
	case SC_CHARSET_DEFAULT: return documentCodePage ? documentCodePage : 1251; // vp: 1252
		...
		case WM_MOUSEWHEEL:
			...
			if (abs(wheelDelta) >= WHEEL_DELTA && linesPerScroll > 0) {
				...
				if (wParam & MK_CONTROL) {
					...
					if( ct.inCallTipMode )	// vp : scroll callTip
					{
						PRectangle r1 = wMain.GetPosition();
						PRectangle r = ct.wCallTip.GetPosition();
						r.Move( 0, vs.lineHeight * -linesToScroll );
						if( r.top<r1.top || r.bottom>r1.bottom )
							ct.CallTipCancel();
						else
							ct.wCallTip.SetPosition( r );
					}					
				}
			}
			return 0;
